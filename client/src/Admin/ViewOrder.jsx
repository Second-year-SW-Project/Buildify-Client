...
import { PageSubtitle, PageTitle } from '../AtomicComponents/Typographics/TextStyles';
...
function ViewOrder() {
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            ...
            try {
                ...
                const response = await axios.get(`${backendUrl}/api/checkout/order/${id}`, {
                    headers: {
    ...
    const handleStepperStatusChange = async (newStatus) => {
        try {
            setLoading(true);
            const response = await axios.patch(`${backendUrl}/api/checkout/product-orders/${id}`, {
                status: newStatus,
            }, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
        ...
        <div className='pl-6 grid grid-rows'>
            ...
            <div className='mt-3'>
                <div className='flex items-center gap-2 '>
                    <PageTitle value={`Order #${order._id?.slice(-4).toUpperCase() || '----'}`} />
                    <Chip
                        label={order.status}
            ...
            <div className="mb-10 gap-4 w-full pr-4 flex items-start">
                <div className='w-2/3'>
                    <div className='orderDetails border-2 border-violet-600 rounded-lg p-4 mb-4'>
                        <Box sx={{ mb: 3 }}>
                            <Typography variant="h6" sx={{ mb: 1, fontWeight: "bold" }}>Order Details</Typography>
                            ...
                            <Box sx={{ mt: 2, textAlign: 'right', mr: 2, display: 'flex', justifyContent: 'flex-end' }}>
                                {/* Label column */}
                                ...
                                <Box sx={{ textAlign: 'right' }}>
                                    ...
                                    <Typography variant="h6" fontWeight="bold" color="warning" sx={{ mb: 1 }}>
                                        + {order.shipping?.toLocaleString() || 0} LKR
                                    </Typography>
                                    <Typography variant="h5" fontWeight="bold">
                                        {((order.total || 0) + (order.shipping || 0)).toLocaleString()} LKR
                                    </Typography>
                                </Box>
                            </Box>
                        </Box>
                    ...
                    <div className='orderTracking border-2 border-violet-600 rounded-lg p-4 mt-4 bg-purple-100'>
                        <Typography variant="h6" sx={{ mb: 2, fontWeight: "bold" }}>Order Tracking</Typography>
                        <OrderStepper
                        ...
                        {order.status === 'Refunded' && (
                            <Box sx={{ mt: 2, textAlign: 'center' }}>
                                <Typography variant="h6" color="ternary" sx={{ fontWeight: "bold" }}>
                                    Order Refunded
                                </Typography>
                                <Typography variant="body2" color="textSecondary">
                                    {order.stepTimestamps?.Refunded ? (
                                        <>
                                            This order has been refunded on {new Date(order.stepTimestamps.Refunded).toLocaleDateString('en-GB', {
                                                day: '2-digit',
                                                month: 'short',
                                                year: 'numeric'
                                            })} at {new Date(order.stepTimestamps.Refunded).toLocaleTimeString([], {
                                                hour: '2-digit',
                                                minute: '2-digit',
                                                hour12: true
                                            })}
                            ...
                            <Box sx={{ mt: 2, textAlign: 'center' }}>
                                ...
                                <Typography variant="body2" color="textSecondary">
                                    {order.stepTimestamps?.Canceled ? (
                                        <>
                                            This order has been Canceled on {new Date(order.stepTimestamps.Canceled).toLocaleDateString('en-GB', {
                                                day: '2-digit',
                                                month: 'short',
                                                year: 'numeric'
                                            })} at {new Date(order.stepTimestamps.Canceled).toLocaleTimeString([], {
                                                hour: '2-digit',
                                                minute: '2-digit',
                                                hour12: true
                                            })}
                            ...
                            <Box sx={{ mt: 2, textAlign: 'center' }}>
                                ...
                                <Typography variant="body2" color="textSecondary">
                                    {order.stepTimestamps?.Delivered ? (
                                        <>
                                            This order has been Delivered on {new Date(order.stepTimestamps.Delivered).toLocaleDateString('en-GB', {
                                                day: '2-digit',
                                                month: 'short',
                                                year: 'numeric'
                                            })} at {new Date(order.stepTimestamps.Delivered).toLocaleTimeString([], {
                                                hour: '2-digit',
                                                minute: '2-digit',
                                                hour12: true
                                            })}
                ...
                <div className='w-1/3'>
                    <div className='userDetails border-2 border-violet-600 rounded-lg p-4'>
                        <Typography variant="h6" sx={{ mb: 2, fontWeight: "bold" }}>Customer</Typography>
                        <Box sx={{
                            display: 'flex',
                            flexDirection: 'column',
                            gap: 2,
                        ...
                        <Typography variant="h6" sx={{ mb: 2, mt: 4, fontWeight: "bold" }}>Shipping</Typography>
                        <Box sx={{ mt: 2, textAlign: 'left', mr: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
                            {/* Address Row */}
                            <Box sx={{ display: 'flex', alignItems: 'flex-start' }}>
                                <Typography variant="body1" fontWeight="bold" color="gray" sx={{ minWidth: '80px' }}>
                                    Address
                                </Typography>
                        ...
                        <Typography variant="h6" sx={{ mb: 2, mt: 4, fontWeight: "bold" }}>Payment</Typography>
